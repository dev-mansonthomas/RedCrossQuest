 ...
 */
    ...
    foreach ($exportData as $tableName => $oneDataTable)
    {
      ...
      else
      {
        ...
        $nbOfLine++;
        $oneDataTable->generateCSVHeader();
        $nbOfLine++;
      }
    ...
    try
    {
      ...
      $z = new \ZipArchive();
      if(true === ($z->open($zipFilePath, \ZipArchive::CREATE | \ZipArchive::OVERWRITE)))
      {
        $z->setPassword($password);
        $z->setArchiveComment( strtr( 'RedCrossQuest Data Export - '.$dateTime .' for UL - '.$ulId.' - '.$exportData['ul']->name.($year!= null? ' for year :'.$year.'':''), $this->unwanted_array ));
